...
void CT_spec_compare(Int_t Runno,TString SimcFileName, TString target){

  TString fileNameM;
  fileNameM = "/net/cdaq/cdaql3data/cdaq/hallc-online/SIMC_ROOTfiles/";
  fileNameM +="coin_replay_production_";
  fileNameM +=Runno;
  fileNameM +="_-1.root";
  ...
  //cointime addition:
	...
	td->GetEntry(event);
	    ...
	    if( !(PgtrBetaMes > 0.5 && PgtrBetaMes < 1.5                       // proton beta cut
		  ...
		  && HcerNpeSum > 0.1 && PhgcerNpeSum > 0.5                    // Cerenkov counter cut for e- and pion
