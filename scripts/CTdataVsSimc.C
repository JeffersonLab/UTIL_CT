...
*/
void CTdataVsSimc(Int_t numofRuns=4,TString SimcFileName, TString target)
{

    gStyle->SetOptStat(0);
    const Int_t runnumber[numofRuns]={3179,3180,3181,3201}; 
   //-----These offsets (for data) to be removed once SHMS is betetr calibrated ----------
    ...
    fileNameM += "../SIMC_ROOTfiles/"
    fileNameM += SimcFileName; //read the root file from SIMC                                                       
    TFile *f2 = new TFile(fileNameM);
    TTree *simc = (TTree*)f2->Get("h666");
    //get the relevant branch                                                                                  
    int nentriesM = simc->GetEntries();
